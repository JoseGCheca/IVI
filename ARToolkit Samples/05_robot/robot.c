...
GLfloat rot1 = 0.0;        // Rotacion para el hombro del robot
GLfloat rot2 = 0.0;        // Rotacion para el codo del robot
...
static void keyboard(unsigned char key, int x, int y) {
  ...
  case 0x1B: case 'Q': case 'q':
    cleanup(); break;
  case 'a': case'A': rot1+=2; break;
  case 'z': case'Z': rot1-=2; break;
  case 's': case'S': rot2+=2; break;
  case 'x': case'X': rot2-=2; break;
  }
}
...
static void draw( void ) {
  ...
  GLfloat matGreen[] = {0.0, 1.0, 0.0, 1.0};
  GLfloat matBlue[] = {0.0, 0.0, 1.0, 1.0};
  GLfloat linksize = 80;  // Tamano del enlace
  ...
  glTranslatef(linksize/2.0, 0.0, 0.0);  // Nos vamos a la mitad
  glPushMatrix();
  glScalef (1.0, 0.7, 0.1);   
  glMaterialfv(GL_FRONT, GL_DIFFUSE, matGreen);
  glutSolidCube(linksize);
  glPopMatrix();
  
  glTranslatef(linksize/2.0,0.0,0.0);  // Avanzamos al extremo del enlace
  glRotatef(rot2, 0.0, 1.0, 0.0);
  glTranslatef(linksize/2.0,0.0,0.0);  // Nos posicionamos en la mitad!
  glPushMatrix();
  glScalef (1.0, 0.7, 0.1);
